...
from peach.common import save_json, load_json, save_list_to_file, load_list_from_file, file_exists, load_tsv
...
class KbDataset(Dataset):
    ...
    def __init__(
        ...
        #print("self.ent_list", self.ent_list) #self.ent_list ['idea_or_concept', 'virus', 'spatial_concept', 'human_caused_phenomenon_or_process',
        #print("self.rel_list", self.rel_list) #self.rel_list ['measures', 'derivative_of', 'disrupts', 'prevents', 'conceptually_related_to',
        #print("self.ent2text", self.ent2text) #self.ent2text {'idea_or_concept': 'idea or concept', 'virus': 'virus', 'spatial_concept': 'spatial concept',
        #print("self.rel2text", self.rel2text) #self.rel2text {'measures': 'measures', 'derivative_of': 'derivative of',
        #print("self.ent2idx", self.ent2idx) #self.ent2idx {'idea_or_concept': 0, 'virus': 1, 'spatial_concept': 2, 'human_caused_phenomenon_or_process': 3
        #print("self.rel2idx", self.rel2idx) #self.rel2idx {'measures': 0, 'derivative_of': 1, 'disrupts': 2, 'prevents': 3, 'conceptually_related_to': 4
        ...
        #print("self.raw_examples", self.raw_examples[:10]) #self.raw_examples [['acquired_abnormality', 'location_of', 'experimental_model_of_disease']]
        #print("self.subj2objs", self.subj2objs)
        '''self.subj2objs defaultdict(<function KbDataset.build_graph.<locals>.<lambda> at 0x7f24401185f0>, {'acquired_abnormality': defaultdict(<class 'set'>, {'location_of': {'experimental_model_of_disease', 'neoplastic_process', 'pathologic_function', 'cell_or_molecular_dysfunction', 'rickettsia_or_chlamydia', 'bacterium', 'fungus', 'mental_or_behavioral_dysfunction', 'virus'}, 'issue_in': {'biomedical_occupation_or_discipline', 'occupation_or_discipline'}'''
        #print("self.obj2subjs", self.obj2subjs)
        ...
        #print(self._sep_id, self._cls_id, self._pad_id) #2 0 1
    ...
    def negative_sampling(self, raw_kg_triplet, weights=None, *args, **kwargs):
        ...
        while True:
            if prob < cdf[0]:
                ...
                src_elem = neg_example[1] # the pos relation itself
                while True:
                    rdm_elem = random.choice(self.rel_list)
                    if src_elem != rdm_elem:
                        break
                assert rdm_elem is not None
                neg_example[1] = rdm_elem # randomly choose a relation which is not the positive example's relation
            if self.pos_triplet_str_set is not None and self._triplet2str(neg_example) in self.pos_triplet_str_set:
                neg_example = [head, rel, tail]
                continue
            else:
    ...
    def str2ids(self, text, max_len=None):
        ...
        return self.tokenizer.convert_tokens_to_ids(wps) # list_of_tokens converted into ids
