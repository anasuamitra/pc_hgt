...
import collections
from pchgt.metric import safe_ranking
...
def train(args, train_dataset, model, tokenizer, eval_dataset=None, eval_fn=evaluate):
    ...
    # print("train_dataset.data_collate_fn", train_dataset.data_collate_fn)
    # learning setup
    ...
    model.zero_grad()
    # print(int(args.num_train_epochs)) # 20
    ...
    for _idx_epoch, _ in enumerate(train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            # print(step) #1-10, 1-100
            # print(batch)
            model.train()
            batch = tuple(t.to(args.device) for t in batch)
            inputs = train_dataset.batch2feed_dict(batch) #TODO
            # print(inputs)
            outputs = model(**inputs)
            # print(outputs)
            '''
                tensor(2.2021, device='cuda:0', grad_fn=<AddBackward0>), tensor([1.1411, 0.6509, 2.8066, 0.8489, 0.6660, 3.4294, 0.5951, 1.1474, 1.0438,<?, ?it/s]
                7.1614, 3.2988, 0.8834, 0.9161, 3.3078, 3.8616, 0.9951],
        ...
        # print(step_loss) #885.4319565296173, 859.5982002019882, 865.4194188117981
            ...
            # if args.max_steps > 0 and global_step > 4: #args.max_steps:
